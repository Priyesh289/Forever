...
const PlaceOrder = () => {
  ...
  const { getCartAmount, navigate, backendURL, token, products, delivery_fee, cartItems, setCartItems } = useContext(ShopContext);
  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex flex-col sm:flex-row justify-between gap-4 pt-5 sm:pt-14 min-h-[80vh] border-t'>
      ...
      <div className='mt-8'>
        ...
        <div className='mt-12'>
          <Title text1={'PAYMENT'} text2={'MEDOTH'} />
          ...
          <div className='flex gap-3 flex-col lg:flex-row lg:flex-wrap'>
            <div onClick={() => setMethod('stripe')} className='flex item-center gap-3 border p-2 px-3 cursor-pointer'>
              ...
              <img className='h-5 w-auto max-w-full object-contain mx-2' src={assets.stripe_logo} alt="" />
            </div>
            <div onClick={() => setMethod('razorpay')} className='flex item-center gap-3 border p-2 px-3 cursor-pointer'>
              <p className={`min-w-3.5 h-3.5 border rounded-full ${method === 'razorpay' ? 'bg-green-400' : ""}`}></p>
              <img className='h-5 w-auto max-w-full object-contain mx-2' src={assets.razorpay_logo} alt="" />
            </div>
            <div onClick={() => setMethod('cod')} className='flex item-center gap-3 border p-2 px-2 cursor-pointer'>
